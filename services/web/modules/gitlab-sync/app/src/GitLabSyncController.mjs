...
import GitLabSyncHandler from './GitLabSyncHandler.mjs'
...
async function getConnectionStatus(req, res) {
  ...
  try {
    const isConnected = await GitLabSyncHandler.getGitConnState(userId)
...
async function getProjectState(req, res) {
  ...
  try {
    const pss = await GitLabSyncHandler.getProjectState(userId, projectId)
    return res.json(pss)
  } catch (err) {
    const info = OError.getFullInfo(err)
    const errStatus  = info?.status || 500
...
async function getUserAndOrgs(req, res) {
  const userId = SessionManager.getLoggedInUserId(req.session)
  const userAndOrgs = await GitLabSyncHandler.getUserAndOrgs(userId)
...
async function listUserRepos(req, res) {
  ...
  try {
    const repos = await GitLabSyncHandler.listUserRepos(userId)
    res.json({ repos })
...
async function getMergeOverview(req, res) {
  ...
  const userId = SessionManager.getLoggedInUserId(req.session)
  try {
    const commitsAndStatus = await GitLabSyncHandler.getMergeOverview(userId, projectId)
...
// Import a GitLab repository as a new project
async function importRepo(req, res) {
  const userId = SessionManager.getLoggedInUserId(req.session)
  const { name, fullName, defaultBranchName } = req.body

  try {
    const projectId = await GitLabSyncHandler.importRepo(userId, name, fullName, defaultBranchName)
    res.json({ projectId })
  } catch (error) {
...
async function oauth2Callback(req, res) {
  ...
  try {
    await TokenManager.saveUserToken(userId, {token, refresh_token, refresh_timestamp})
  } catch (err) {
    const info = OError.getFullInfo(err)
    const errStatus  = info?.status || 500
    logger.error(OError.getFullStack(err))
  ...
  req.session.projectSyncSuccessMessage =
    req.i18n.translate('gitlab_successfully_linked_description')
  res.redirect('/user/settings?oauth-complete=gitlab#project-sync')
}
...
async function exportProject(req, res) {
  ...
  try {
    await GitLabSyncHandler.exportProject(userId, projectId, req.body)
    ...
    let key = 'gitlab_validation_check'
    if (err instanceof AlreadyExistsError) key = 'gitlab_validation_name_exists'
    else if (errStatus === 401 || errStatus === 403) key = 'gitlab_validation_check_auth'
...
async function gitMerge(req, res) {
  ...
  try {
    ...
    const errStatus  = info?.status || 500
    logger.error(OError.getFullStack(err))
    logger.error({ info, projectId }, 'Error syncing project to GitLab')
    return res.status(errStatus).json({ message: err.message })
  }
}
...
async function unlinkRepo(req, res) {
  ...
  const userId = SessionManager.getLoggedInUserId(req.session)
  try {
    const ownerEmail = await GitLabSyncHandler.unlinkRepo(userId, projectId)
